...
def dvdq_vector_page():
    ...
    def process_data(df, matched_array):
        voltage_array = df['Voltage(V)'].values
        matching_indices = []
        i = 0
    ...
    if uploaded_file is not None:
        ...
        if option.lower() == 'charge':
            new_df = process_data(data, charge_matched_array)
            st.dataframe(new_df)
            fig = go.Figure()
            ...
            fig.update_layout(title='Capacity(Ah) vs dvdq and Gaussian_smooth', 
                            xaxis_title='Capacity(Ah)', 
                            yaxis_title='dvdq and Gaussian_smooth')
            st.plotly_chart(fig)
            ...
            new_df = process_data(data, discharge_matched_array)
            st.dataframe(new_df)
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=new_df['Capacity(Ah)'], y=new_df['dvdq'], mode='lines', name='dvdq'))
            fig.add_trace(go.Scatter(x=new_df['Capacity(Ah)'], y=new_df['Gaussian_smooth'], mode='lines', name='Gaussian_smooth'))
